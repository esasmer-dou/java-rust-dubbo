...
[![Sürüm](https://img.shields.io/badge/sürüm-0.7.3-blue.svg)](https://github.com/esasmer-dou/java-rust-dubbo/releases/tag/v0.7.3)
...
Güncel uyumlu sürüm çizgisi `java-rust-dubbo:0.7.3` ve `rust-java-rest:4.5.5` şeklindedir. Birden
...
`29`, Redis ABI `6` ve Glowroot ABI `3` kullanır. Bu rehberde gösterilen yayınlanmış Maven sürümü
`rust-java-rest:4.5.5` değeridir. Aynı koordineli ABI hattında paketlenen native artifact'i kullanın.
...
- Yalnızca derlemede kullanılan `codegen` classifier içindeki `com.reactor.rust.dubbo.codegen.GenerateNativeDubboClient`
...
Annotation processor, `CatalogClient.create(client, support)` metodunu ve Dubbo interface'indeki her
metot için doğrudan bir async metot üretir. `byte[]` dönüş tipi için ayrıca `...NativeJsonAsync()`
metodu oluşur. Bu yol response body'yi Rust native memory içinde tutabilir. Metot descriptor'ları,
dönüş tipleri, group, version, retry politikası ve metrics erişimi build sırasında sabitlenir. Üretilen
wrapper dinamik proxy yolunu kullanmaz. `NativeDubboConsumerClient`, Java interface'indeki `Method`
nesnelerini client oluşturulurken bir kez çözer. Request sırasında proxy dispatch, metot adına göre
arama veya reflection çalışmaz.
...
`DubboServiceBinding.service(...)` yalnızca startup metadata'sıdır. Implementation'ın interface ile
uyumunu doğrular ve Dubbo başlamadan önce açık servis listesini kaydeder. Provider çağrısına ek wrapper,
lookup, allocation veya dispatch katmanı eklemez.
...
### 4. Native Client'i Build Sırasında Üretin (Önerilen Yol)

Küçük build-only classifier'ı consumer uygulamasına ekleyin. `provided` scope, processor sınıflarının
runtime dependency yüzeyine girmesini önler:
...
endpoint yalnızca provider JSON'unu iletiyorsa body Java heap üzerinde `byte[]` olarak oluşmaz.

BEST: Sabit Dubbo interface'leri için generated client kullanın. El yazımı descriptor ve retry kodunu
azaltır; runtime magic eklemez. ACCEPTABLE: Dinamik veya özel embedded entegrasyonlarda aşağıdaki
manuel API kullanılabilir. ANTI-PATTERN: Request başına reflective proxy oluşturmak veya metot
descriptor'ı çözmek.

Generator, üst interface'lerden miras alınan metotları da üretir. Sözleşme kontrolü derleme sırasında
yapılır. Overload edilmiş metotlar, `void` metotlar, generic service interface'leri ve çözümlenmemiş
generic metot tipleri açık bir derleme hatasıyla reddedilir. Dubbo taşıma sözleşmesini net ve generic
olmayan tiplerle kurun. Concrete record, primitive, `String`, collection veya `byte[]` kullanın.

### 5. Manuel Client Kurulumu (Desteklenen Alternatif)
...
- `target/java-rust-dubbo-0.7.3.jar`
- `target/java-rust-dubbo-0.7.3-native-static.jar`
- `target/java-rust-dubbo-0.7.3-codegen.jar` (yalnızca derleme sırasında kullanılır)
- `target/java-rust-dubbo-0.7.3-sources.jar`

Sürüm ayrıntıları: [java-rust-dubbo 0.7.3](docs/RELEASE_NOTES_v0.7.3.tr.md).
